...
- **Large-result notification:** `base_agent/__init__.py:_maybe_notify_large_tool_result()` is called from `BaseAgent._on_tool_result_hook()` on every main-agent tool result. `ToolExecutor` passes `tool_call_id` directly to the hook (no heuristic scan needed); a fallback scan of pending assistant ToolCallBlocks is used only when the id is absent. It checks `_summarize_notification_threshold` (default 5000 chars) and excludes `daemon_tool_result` tool names (child-daemon relays). For spill manifests it uses `original_char_count` as the effective length: if the original exceeded the threshold, a spill-specific notification including the sidecar path is published; if `original_char_count` is missing or under threshold, no notification fires. The bare `daemon` tool is NOT excluded — it is a normal main-agent capability. Agents may adjust the threshold at runtime via `system(action="summarize", notification_threshold_chars=N)` (0 = disable; returned as `notification_threshold_chars` in every summarize response). The hook is invoked in both the sequential and parallel execution paths (phase 3 of `_execute_parallel`).
