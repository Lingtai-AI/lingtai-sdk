...
- `_molt.py` — Context molt core, name handlers, and system-initiated molt.
  ...
  - `_publish_post_molt()` (`_molt.py`) — internal helper. Writes `.notification/post-molt.json` via `publish_notification` (channel `post-molt`, icon 🌱, priority high). Payload carries `molt_id`, `molt_at`, `source_agent`, `initiator`, `source`, `molt_count`, `reminder`, best-effort display-only `next_action`, `ack_options`, `reasoning` (agent molts only), `summary_path`, and `tokens_before/after`. Instructions tell the agent to consult pad/summary/human channels and explicitly choose continue/defer/obsolete, then dismiss via `system(action='dismiss', channel='post-molt', reason='<choice>: ...')`. The channel is intentionally separate from the pressure `molt` channel owned by `base_agent.turn._check_molt_pressure` so pressure-clear under threshold never sweeps the reminder.
