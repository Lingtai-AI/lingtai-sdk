...
| `post-child-delegation.json` | `intrinsics/psyche/_molt.py` (one-shot post-molt awareness snapshot of direct avatar ledger entries and active daemon run dirs; best-effort, no lifecycle actions) | bare kernel channel |
...
- `.notification/<tool>.json` — notification dropbox (one file per producer channel — `email.json`, `soul.json`, `system.json`, `post-child-delegation.json`, `mcp.<server>.json`). Polled by `BaseAgent._sync_notifications` on every heartbeat tick. See "Notifications" section above.
