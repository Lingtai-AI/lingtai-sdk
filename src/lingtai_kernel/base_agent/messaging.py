...
def _rerender_unread_digest(agent) -> str | None:
    ...
    publish_notification(
        ...
        instructions=(
            ...
            "a slow bash, daemon, web_search, or large file write), "
            "attach the email follow-up to that primary call via the "
            "reserved 'secondary' field instead of leaving the sender waiting: "
            "use secondary={tool:'email', args:{action:'reply', "
            "email_id:[id], message:'I am checking now...'}} for a quick "
            "acknowledgement, or secondary={tool:'email', args:{action:'read', "
            "email_id:[...]}} when a preview is truncated and you need the "
            "full body before the primary starts. The bounded read result is "
            "forwarded under _secondary.result on the primary tool result. "
