...
| `CodexResponsesSession` | ~1571–1840 | Stateless Responses variant for ChatGPT-OAuth `/backend-api/codex`; sends stable `session-id` / `thread-id` REST cache-affinity headers (`_cache_affinity_headers`), passed down by default from the agent path (one 8-char hash, byte-identical for both); copies the resolved ids into `UsageMetadata.extra` for `token_ledger.jsonl` |
| `CodexOpenAIAdapter` | ~1842–1965 | Adapter variant that builds `CodexResponsesSession`; resolves the per-agent cache-affinity value from the anchor (or explicit `codex_session_id`); `_resolve_codex_ids` returns `(id, id)` — thread tracks session — and `(None, None)` only for a bare/test adapter; `_default_prompt_cache_key` returns the SAME per-agent value (falls back to `lingtai-codex:{model}:v1` only on the bare/no-anchor path) |
