...
2. **10-call cache corruption** — the backend returns the SAME positive `cached_tokens` for `_CODEX_CACHE_AFFINITY_ROTATE_QUEUE_LEN` (= 10) requests in a row; the session re-derives a fresh current id (same helper, new epoch) and keeps using it for ALL subsequent requests (persistent, no revert).
...
- `_record_cache_hit(cached_tokens, model=...)` runs after each `response.completed`. It appends **only positive** `cached_tokens` (a `0` is a miss, not a stalled hit) to a rolling queue of length `_CODEX_CACHE_AFFINITY_ROTATE_QUEUE_LEN` (= 10). When the queue is full and every value is byte-identical (`len(set(...)) == 1`), it calls `_rotate_current_affinity` and clears the queue (a fresh run of ten is required to re-fire).
