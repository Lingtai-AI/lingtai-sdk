...
| `_codex_identity_headers()` | ~168–240 | Codex client-identity headers sent on every Codex request. **Default flipped to impersonate the official Codex CLI** (#471 cache-miss experiment): `{originator: lingtai, User-Agent: lingtai/<version> (os; arch)}`, matching the installed `codex-cli 0.130.0` shape. Single reversible switch `_CODEX_IMPERSONATE_OFFICIAL_CLI`; set `False` to restore the honest `lingtai`/`LingTai/<version>` identity (#436). Prior A/B found identity cache-irrelevant — this re-tests that |
