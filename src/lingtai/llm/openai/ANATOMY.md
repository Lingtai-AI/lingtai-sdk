...
| `_codex_identity_headers()` | ~168–240 | Codex client-identity headers sent on every Codex request. **Default flipped to impersonate the official Codex CLI** (#471 cache-miss experiment): `{originator: codex_cli_rs, User-Agent: codex_cli_rs/<version> (os; arch)}`, matching the installed `codex-cli 0.130.0` shape. Single reversible switch `_CODEX_IMPERSONATE_OFFICIAL_CLI`; set `False` to restore the honest `lingtai`/`LingTai/<version>` identity (#436). Prior A/B found identity cache-irrelevant — this re-tests that |
...
**Codex-only (#436 → #471 experiment).** Every Codex request carries identity headers — `originator` and `User-Agent` (`_codex_identity_headers()`) — merged as the base layer of `extra_headers` so cache-affinity and caller-supplied headers still override. The **current default impersonates the official Codex CLI app name**: `originator: codex_cli_rs` and `User-Agent: codex_cli_rs/<version> (os; arch)`, matching the installed `codex-cli 0.130.0` (`codex_login::auth::default_client`; the `exec` subcommand instead overrides to `codex_exec`). This is the cache-miss experiment requested 2026-06-22: the hypothesis is that the ChatGPT backend cold-slots an unrecognized originator. A single module flag `_CODEX_IMPERSONATE_OFFICIAL_CLI` flips the whole identity back to honest LingTai (`originator: lingtai`, `User-Agent: LingTai/<version>`). **Caveat:** the prior #436 A/B found identity headers do **not** affect prompt caching (identical cache rate with or without — only stable `session_id`/`thread_id` route the slot); this experiment re-tests that. If the cache rate is unchanged, flip the switch back (impersonating a first-party client otherwise carries ToS risk).
