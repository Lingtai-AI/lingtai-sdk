...
**`agent.py`** — `Agent(BaseAgent)`: `__init__` :33 (accept `capabilities=` + `disable=`, expand groups, `apply_core_defaults`, decompress addons, setup caps, install manuals, load MCP) · `_setup_capability` :156 · `_persist_llm_config` :131 · `_install_intrinsic_manuals` :178 · `_load_mcp_from_workdir` :384 (also tracks specs in `_mcp_init_specs`) · `_retry_failed_mcps` :536 (re-spawn dead MCPs on `system(refresh)` — issue #34) · `_read_init` :892 (runs `lingtai_kernel.migrate.run_agent_migrations()` before reading `init.json`, then materializes preset, strips plain deprecated fields, validates, resolves paths including `prompt_file`, and publishes the resolved manifest to `system/manifest.resolved.json` via `lingtai_kernel.workdir.write_resolved_manifest` — issue #259) · `_setup_from_init` :1048 (**full reconstruct** — shared by boot and live refresh; reads `manifest.disable` and re-applies `apply_core_defaults`) · `_activate_preset` :974 (runtime swap, atomic write) · `_reload_prompt_sections` :1293 (writes base `prompt` from inline `prompt` / resolved `prompt_file`, plus `covenant`/`substrate`/`rules`/`principle`/`procedures`/`brief`/`comment`; delegates `character` to `_lingtai_load` and `pad` to `_pad_load` — the canonical composers — so boot/refresh/molt are consistent and hook-order-independent) · `connect_mcp` :757 / `connect_mcp_http` :812 · `start` :750 / `stop` :861
...
| Path | When | What |
|---|---|---|
| `<workdir>/init.json` | `_activate_preset` :974, `cli.run` :202, `_read_init` :892 | Preset swap (atomic write); venv_path writeback; boot/refresh cleanup of deprecated top-level fields, archive-preserving init migrations, and path resolution including `prompt_file` → inline `prompt` |
| `<workdir>/system/llm.json` | `_persist_llm_config` :127 | LLM provider/model/base_url for revive |
| `<workdir>/system/manifest.resolved.json` | `_read_init` :892 via `lingtai_kernel.workdir.write_resolved_manifest` | Derived runtime artifact (issue #259): fully-resolved manifest (preset materialized, validated, paths resolved including `prompt_file`) with secret-bearing keys removed, plus `schema`/`generated_at`/`source`/`preset` metadata. Atomic write, regenerated on every boot/refresh/molt-reload; init.json is never written back. |
| `<workdir>/system/{covenant,principle,substrate,procedures,brief,rules,pad,lingtai}.md` + `pad_append.json` | `_reload_prompt_sections` :1293 | Prompt sections from init.json + disk. Inline `prompt` / resolved `prompt_file` is stored as the base prompt before dynamic sections; `covenant.md`→`covenant`, `lingtai.md`→`character` (via `_lingtai_load`), `pad.md`+`pad_append.json`→`pad` (via `_pad_load`). `character` is the agent's self-authored identity (灵台), distinct from `covenant` (operator contract) and the mechanical `identity` section. `substrate` is kernel-owned, cross-app stable (issue #39); kept compact and routed to the packaged `system-manual` skill for expanded operating guidance; auto-seeded from packaged `lingtai/prompts/substrate.md` on first boot if neither `data["substrate"]` nor `system/substrate.md` provides content — see `_setup_from_init` :1048. |
